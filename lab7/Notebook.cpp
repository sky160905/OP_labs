...
void Notebook::isValidPhoneNumber(const char* phoneNumber) {
    ...
    if (strlen(phoneNumber) != 9) {
        throw std::invalid_argument("Phone number error: Invalid amount of symbols, should be 9");
    }
...
void Notebook::isValidDateOfBirth(const char* dateOfBirth) {
    ...
    if (strlen(dateOfBirth) != 10) {
        throw std::invalid_argument("Date of birth error: Invalid amount of symbols");
    }
    if (day < 1 || day > 31 || month < 1 || month > 12 || year < 1000 || year > 2023) {
        throw std::invalid_argument("Date of birth error: Date does not exist");
    }
...
void Notebook::isValidName(const char* name) {
    ...
    if (wordCount > 3) {
        throw std::invalid_argument("Name error: No more than 3 words");
    }

    for (char c : nameString) {
        if (isdigit(c)) {
            throw std::invalid_argument("Name error: Name consists of letters only");
        }
    }
}
...
std::string Notebook::checkDuplicatePeople(const Notebook& otherNotebook) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < otherNotebook.count; j++) {
            if (stricmp(people[i].getName(), otherNotebook.people[j].getName()) == 0 &&
        ...
        if (isDuplicate) {
            ...
            contents += "Phone Number: +380" + std::string(people[i].getPhoneNumber()) + " \r\n";
...
std::string Notebook::getFileContents() {
    ...
    while (!file.eof() && fileCount < count) {
        ...
        contents += "Phone Number: +380" + std::string(people[fileCount].phoneNumber) + " \r\n";
